...
async def on_private_text_antispam_add(message: Message):
    """Обработка ввода user_id для добавления в антиспам базу (после нажатия «Добавить по ID»)."""
    if not message.from_user or message.from_user.id not in _pending_antispam_add:
        return
