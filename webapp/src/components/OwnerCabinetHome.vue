...
/** Золотой бейдж «Premium» только в Free-кабинете (апселл). */
...
<template>
  <div class="space-y-3 font-display">
    ...
    <div class="grid grid-cols-2 gap-2.5">
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-emerald-500/35 bg-gradient-to-br from-emerald-950/80 via-emerald-900/40 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_28px_-14px_rgba(16,185,129,0.55)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('protection')"
      >
        <span
          v-if="showPremiumUpsellBadge"
          class="absolute right-2 top-2 inline-flex items-center gap-0.5 rounded-md border border-amber-400/40 bg-gradient-to-r from-amber-400/95 via-yellow-300/90 to-amber-500/95 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-amber-950 shadow-sm"
        >
        ...
        <p class="mt-1 text-[11px] text-white/55">Удалено</p>
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-indigo-500/35 bg-gradient-to-br from-indigo-950/85 via-violet-950/50 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_28px_-14px_rgba(99,102,241,0.5)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('stats')"
      >
        <span
          v-if="showPremiumUpsellBadge"
        ...
        <p class="mt-1 text-[11px] text-white/55">Подписалось</p>
        <div class="pointer-events-none absolute bottom-2.5 right-2.5 opacity-95">
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-sky-500/30 bg-gradient-to-br from-sky-950/75 via-blue-950/40 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_26px_-14px_rgba(14,165,233,0.45)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('updates')"
      >
        <p class="text-[13px] font-bold text-white">Обновления</p>
        <p class="mt-1.5 text-[11px] leading-snug text-sky-100/75">Новые функции и улучшения</p>
        <div class="pointer-events-none absolute bottom-2.5 right-2.5 opacity-95">
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-amber-500/35 bg-gradient-to-br from-amber-950/80 via-orange-950/45 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_26px_-14px_rgba(245,158,11,0.4)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('partner')"
      >
        <p class="text-[13px] font-bold text-white">Партнёрство</p>
        <p class="mt-1.5 text-[11px] leading-snug text-amber-100/75">Зарабатывай с нами</p>
        <p v-if="referralPaidCount != null" class="mt-2 text-xl font-extrabold text-amber-100">{{ referralPaidCount }} платящих</p>
        <div class="pointer-events-none absolute bottom-2.5 right-2.5 opacity-95">
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-fuchsia-500/35 bg-gradient-to-br from-fuchsia-950/80 via-purple-950/45 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_26px_-14px_rgba(217,70,239,0.45)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('broadcasts')"
      >
        <p class="text-[13px] font-bold text-white">Рассылки</p>
        <p class="mt-1.5 text-[11px] leading-snug text-fuchsia-100/75">Управление рассылками</p>
      ...
      <button
        type="button"
        class="relative overflow-hidden rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-950/75 via-teal-950/40 to-slate-950/90 p-3.5 pb-11 text-left shadow-[0_0_26px_-14px_rgba(34,211,238,0.4)] ring-1 ring-white/[0.05] transition hover:brightness-105 active:scale-[0.99]"
        @click="open('settings')"
      >
        <p class="text-[13px] font-bold text-white">Настройки</p>
        <p class="mt-1.5 text-[11px] leading-snug text-cyan-100/75">Гибкая настройка сервиса</p>
        <div class="pointer-events-none absolute bottom-2.5 right-2.5 opacity-95">
